    ...
    try {
      if (isLoginMode) {
        ...
        if (!data.name || data.name.length < 2) {
          setAuthError('שם חייב להכיל לפחות 2 תווים');
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
          ...
          <p className="text-gray-600 font-medium">התחברת בהצלחה. מעביר אותך ללוח הבקרה...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="mx-auto w-full max-w-sm lg:w-96">
          <div>
            <h2 className="text-3xl font-extrabold text-gray-900">
              {isLoginMode ? 'התחברות למערכת' : 'הצטרפות כסופר חדש'}
            </h2>
            <p className="mt-2 text-sm text-gray-600 font-medium">
              ...
              <button 
                ...
                className="font-bold text-green-600 hover:text-green-500 bg-transparent border-none cursor-pointer p-0"
              >
                {isLoginMode ? 'צור חשבון חדש' : 'התחבר כאן'}
              </button>
            </p>
          </div>
          ...
          <div className="mt-8">
            {authError && (
              <div className="mb-4 bg-red-50 text-red-600 p-3 rounded-lg text-sm font-bold border border-red-100">
                {authError}
              </div>
            )}
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div className="pt-2">
                ...
                >
                  {isSubmitting ? 'מעבד...' : (isLoginMode ? 'התחבר לחשבון' : 'צור חשבון סופר')}
                </button>
              </div>
            </form>
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-2 bg-white text-gray-500 font-medium">
                    {isLoginMode ? 'או התחבר באמצעות' : 'או הרשמה מהירה'}
                  </span>
                </div>
              </div>
